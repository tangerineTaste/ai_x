...
def get_naver_api_data(media, word):
    "w  ord에 대해 media 검색한 결과의 str을 return"
    load_dotenv()
    ...
    url = f"https://openapi.naver.com/v1/search/{media}?sort=date&display=20&query={encText}" # JSON 결과
...
def ai_analysis(wb):
    ...
    # 3. 프롬프트 작성
    prompt = f"""다음 두 목록을 비교분석하여, prev_list 목록에서 now_list목록으로 바뀐 주요 특징을 추출해줘. 
                prev_list 목록: {prev_data}
                now_list 목록: {now_data}
                비교 분석 결과를 바탕으로 구체적인 수치, 상품명, 쇼핑몰명 등을 언급하여 한글자로 100저 이내로 분석글을 작성해줘"""
    
    return get_ai_summrize(prompt)

def get_openai_news_summarize(str_news_data):
    prompt = f"""다음 뉴스 내용을 구체적인 수치, 고유명사를 언급하며 글머리를 활용하여 한글 200자 이내로 요약 글을 작성해줘.
    뉴스 내용 : {str_news_data}"""
